  ...
  installPlatform(projectDirectory: string): Q.Promise<void> {
    ...
    const gradleContent: string = slash(
      path.join(
        innerprojectDirectory,
        "node_modules",
        "@recodepush/react-native-code-push",
        "android",
        "codepush.gradle"
      )
    );
...
class RNProjectManager extends ProjectManager {
  ...
  public setupProject(
    ...
    if (TestConfig.isExpoApp) {
      return TestUtil.getProcessOutput(
        `npx create-expo-app@latest ${appName} --template blank`,
        { cwd: projectDirectory, timeout: 30 * 60 * 1000 }
      )
        .then((e) => {
          console.log(
            `"npx expo init ${appName}" success. cwd=${projectDirectory}`
          );
          return e;
        })
