    ...
    private void downloadAndApplyMultiplePatches(JSONArray patches, String finalUpdateFolderPath,
        ...
        try {
            ...
            for (int i = 0; i < totalPatches; i++) {
                ...
                if (tempResultDir.exists() && tempResultDir.isDirectory()) {
                        ...
                        for (File file : files) {
                            if (file.isDirectory()) {
                                FileUtils.copyDirectoryContents(
                                    ...
                                    CodePushUtils.appendPathComponent(workingFolderPath, file.getName())
                                );
                            } else {
                                File destFile = new File(workingFolderPath, file.getName());
